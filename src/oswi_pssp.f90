...
! Get oswi input file
!
  include 'oswi_pssp_command_args.f90'
...
!
  if (verb) then
    write (output_unit,"(a)")  ''
    write (output_unit,"(3a)") 'OSWI_PSSP v', trim(liboswi_version()), &
      ' - OSWI Pseudo Spectra'
    write (output_unit,"(a)")  ''
  end if
!
...
! check
!
  if (count(infs_mask).lt.1_int32.or.oswi%hdr%integrate) then
    write (error_unit,"(a)") 'Error : no files with oswi spectral data found!'
    stop
  end if
!
...
Contains
  ...
  include 'oswi_pssp_help.f90'

End program oswi_pssp
